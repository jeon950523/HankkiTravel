  ...
  <div class="content-page journey-page">
    ...
    <template v-else>
      ...
      <section v-if="selectedSlot?.anchor && ['LUNCH', 'DINNER'].includes(selectedSlot.mealType)" :id="`decision-${dessertSlotForMeal(selectedSlot.mealType)}`" class="surface next-decisions" aria-label="식후 디저트">
        ...
        <div class="candidate-grid"><article v-for="candidate in dessertResult.candidates || []" :key="candidate.contentId" class="surface live-card candidate-card"><KtoImage :src="candidate.imageUrl" :alt="imageAlt(candidate.title)" /><div class="live-card-body"><p class="card-kicker">{{ dessertExplorationMode === 'SEARCH' ? '직접 검색한 디저트' : dessertExplorationMode === 'OTHER' ? '주변의 다른 디저트' : '선택형 식후 디저트' }}</p><h3>{{ candidate.title }}</h3><p class="address">{{ candidate.address }}</p><p v-if="candidate.distanceMeters != null">식사 장소에서 직선거리 {{ candidate.distanceMeters < 1000 ? `${candidate.distanceMeters}m` : `${(candidate.distanceMeters / 1000).toFixed(1)}km` }}</p><p>{{ (candidate.reasons || candidate.fitReasons || []).join(' ') }}</p><p v-if="candidate.telephone">전화 {{ candidate.telephone }}</p><p>{{ (candidate.cautions || candidate.checkBeforeVisit || []).join(' ') }}</p><div class="recommendation-actions"><a class="action-button button-secondary" :href="kakaoSearchUrl(candidate.title)" target="_blank" rel="noopener noreferrer">카카오맵에서 검색</a><a class="action-button button-secondary" :href="candidateDirections(candidate, dessertResult).href" target="_blank" rel="noopener noreferrer">{{ candidateDirections(candidate, dessertResult).label }}</a></div><p class="source-line">{{ candidate.sourceAttribution }}</p><button class="action-button button-primary" type="button" :disabled="Boolean(trips.actionLoading)" @click="selectDessert(candidate)">이 디저트 선택</button></div></article></div>
